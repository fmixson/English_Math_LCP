...
df = pd.read_csv('C:/Users/fmixson/Desktop/Dashboard_files/LCP_English_Math/campus-v2report-enrollment-2023-05-23(3).csv')
# df = pd.read_csv(
#     'C:/Users/fmixson/Desktop/Dashboard_files/LCP_English_Math/campus-v2report-enrollment-2023-05-11 (1).csv')
df.to_excel('test1.xlsx')
pd.set_option('display.max_columns', None)
df = df[df['Dropped?'] == 'No']
sorted_df = df.sort_values(by='Student ID').reset_index()
sorted_df.to_excel('test.xlsx')
id_list = []
print(df)
for i in range(len(df)):
    if sorted_df.loc[i, 'Student ID'] not in id_list:
        id_list.append(sorted_df.loc[i, 'Student ID'])

for id in id_list:
    student_df = sorted_df[sorted_df['Student ID'] == id]
